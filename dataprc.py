...
# ----------------------------------------------------------------------
# 1. Mapeamento de Gêneros e Ordem
# ----------------------------------------------------------------------
...
GENERO_MAP = {
    ...
    # Poesia
    'jó': 'Poesia', 'sl': 'Poesia', 'pv': 'Sabedoria', 'ec': 'Sabedoria', 'ct': 'Sabedoria', 
...
TESTAMENTO_ANTIGO = {
    'Pentateuco', 'Historia', 'Poesia', 'Sabedoria', 
...
def clean_and_count_words(text):
    """Conta palavras em um texto, ignorando pontuação."""
...
def process_bible_json(json_file_path, output_csv_path):
    """
    Carrega o JSON, calcula as palavras e salva no formato CSV.
    """
    try:
    ...
    for livro_data in dados_biblia:
        ...
        genero = GENERO_MAP.get(abrev_livro, 'Outros') # 'Outros' se a abreviação não for encontrada
        ...
        if 'chapters' in livro_data:
            ...
            for index, versiculos_list in enumerate(capitulos_data):
                ...
                palavras = clean_and_count_words(texto_capitulo)
...
if __name__ == '__main__':
    ...
    process_bible_json(JSON_ENTRADA, CSV_SAIDA)
